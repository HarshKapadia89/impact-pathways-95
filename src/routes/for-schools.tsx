...
import { Badge, Button, Card, Field, Input, Textarea } from "@/design-system/hbk-career-brand-guidelines-4f1c39";
...
function ForSchoolsPage() {
  ...
  return (
    <PublicLayout>
      {/* HERO */}
      <StudentPhotoHero tone="brand" image={planningStudents} imageAlt="Indian school students learning and planning together" eyebrow={L("For Schools", "શાળાઓ માટે")} title={L("Bring evidence-based career guidance to every student.", "તમારી શાળાના દરેક વિદ્યાર્થી માટે પુરાવા-આધારિત કારકિર્દી માર્ગદર્શન.")} subtitle={L(
      ...
      <section id="what-you-get" className="max-w-7xl mx-auto px-4 md:px-8 py-14">
        ...
        <div className="grid md:grid-cols-3 gap-4 mt-8">
          {[
            ...
            <Card key={f.t} variant="arrow" padding="md">
        ...
        {done ? (
          ...
          <form onSubmit={submit} className="mt-6 rounded-xl border border-border bg-card p-6 space-y-4">
            <div className="grid md:grid-cols-2 gap-3">
              <Field htmlFor="school-name" label={L("School name *", "શાળાનું નામ *")}><Input id="school-name" required value={form.school_name} onChange={(e) => setForm({ ...form, school_name: e.target.value })} /></Field>
              <Field htmlFor="school-city" label={L("City", "શહેર")}><Input id="school-city" value={form.city} onChange={(e) => setForm({ ...form, city: e.target.value })} /></Field>
              <Field htmlFor="contact-person" label={L("Contact person *", "સંપર્ક વ્યક્તિ *")}><Input id="contact-person" required value={form.contact_person} onChange={(e) => setForm({ ...form, contact_person: e.target.value })} /></Field>
              <Field htmlFor="contact-role" label={L("Role (Principal, Coordinator...)", "પદ (પ્રિન્સિપાલ, સંકલક...)")}><Input id="contact-role" value={form.role} onChange={(e) => setForm({ ...form, role: e.target.value })} /></Field>
              <Field htmlFor="contact-email" label={L("Email *", "ઇમેલ *")}><Input id="contact-email" required type="email" value={form.email} onChange={(e) => setForm({ ...form, email: e.target.value })} /></Field>
              <Field htmlFor="contact-phone" label={L("Phone", "ફોન")}><Input id="contact-phone" value={form.phone} onChange={(e) => setForm({ ...form, phone: e.target.value })} /></Field>
              <Field htmlFor="student-count" label={L("Approx. student count (Class 6–12)", "આશરે વિદ્યાર્થી સંખ્યા (ધોરણ 6-12)")}><Input id="student-count" type="number" min="0" value={form.student_count} onChange={(e) => setForm({ ...form, student_count: e.target.value })} /></Field>
            </div>
            <Field htmlFor="school-message" label={L("Anything specific to discuss?", "ચર્ચા માટે કંઈ ખાસ?")}>
              <Textarea id="school-message" rows={3} value={form.message} onChange={(e) => setForm({ ...form, message: e.target.value })} />
            </Field>
